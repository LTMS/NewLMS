...
function teamsheet_dept()
	...
	function checkDate(){
		$result= $this->input->post();
		//echo $this->timesheet_model->checkDate($result["d1"]);
		echo $result["d1"];
	}
	function checkLeave(){
		$result= $this->input->post();
		echo $this->timesheet_model->checkLeave($result["d1"]);
	}

	function checkLocked(){
		$result= $this->input->post();
		echo $this->timesheet_model->checkLocked($result["d1"]);
	}

	function get_INOUT(){
		$result= $this->input->post();
		$col=$this->timesheet_model->get_INOUT($result["d1"]);
		echo $col;
	}
