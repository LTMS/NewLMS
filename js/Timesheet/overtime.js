			...
			function get_my_ot(){
				...
				var month1=document.getElementById('month').value;
					//alert(month1);
				if(year1!="" && month1!=""){
					$.post(site_url+"/Timesheet/overtime/get_my_ot/",{year:year1,month:month1},function(data){		
						//alert(data);
						$("#contentData").html("");
		...
		function get_admin_otsummary(){
				...
				if(year!="" && month1!="" && user!="" && month2!=""){
					$.post(site_url+"/Timesheet/overtime/get_admin_otsummary/",{d1:date1,d2:date2,emp:user},function(data){		
						//alert(data);
						$("#contentData").html("");
						$("#contentData").append(data);
		...
		function acknowledge_OT(){
			...
			if(year!="" && month1!="" && user!="" && month2!=""){
					...
					if(ot_sum>0){
						$.post(site_url+"/Timesheet/overtime/check_acknowledged/",{user:emp,d1:date1,d2:date2},function(data){		
							//alert(data);
							var str=data.split('::');
							var count=str[0];
							var from=str[1];
									...
									if(count==0){	
										var amount1=prompt("Enter Amount or Remark..!","");
												if(amount1!="" || amount1.length>0){
															$.post(site_url+"/Timesheet/overtime/acknowledge_OT/",{user:emp,d1:date1,d2:date2,ot_hrs:ot_sum,amount:amount1},function(data1){		
		...
		function ack_history_dept(){
			...
			if(year!="" && month1!="" && dept1!="" && month2!=""){
				$.post(site_url+"/Timesheet/overtime/ack_history_dept/",{d1:date1,d2:date2,dept:dept1},function(data){		
					//alert(data);
					$("#contentData").html("");
					$("#contentData").append(data);
		...
		function ack_history_emp(){
				...
				if(year!="" && month1!="" && emp!="" && month2!=""){
					$.post(site_url+"/Timesheet/overtime/ack_history_emp/",{year:year1,user:emp},function(data){		
						//alert(data);
						$("#contentData").html("");
						$("#contentData").append(data);
	...
	function printOT_Summary(){
				...
				if(dept!=""){
					docprint.document.write("<p align='center' style='font-size:18px;color: black;font-weight:bolder;'><u> Over Time Summary of "+dept+" Department</u></p>");
					docprint.document.write("<p align='center' style='font-size:18px;color: black;font-weight:bolder;'><u> "+month+"-"+year+"</u></p>");
				}
				docprint.document.write("");
				docprint.document.write(content_value);
			...
			function admin_ot_dept(){
				...
				if(year!="" && month1!="" && dept1!="" && month2!=""){
					$.post(site_url+"/Timesheet/overtime/admin_ot_dept/",{d1:date1,d2:date2,dept:dept1},function(data){		
						//alert(data);
						$("#contentData").html("");
						$("#contentData").append(data);
		...
		function cancel_Acknowledged(id1){
			...
			var reason=confirm("Do You want to cancel this OT - Acknowlegement?");
				if(reason==true){
					$.post(site_url+"/Timesheet/overtime/cancel_Acknowledged/",{id:id1},function(data){		
			...
			function get_admin_ot(){
				...
				if(year1!="" && month1!="" && user!=""){
					$.post(site_url+"/Timesheet/overtime/get_admin_ot/",{year:year1,month:month1,emp:user},function(data){		
						//alert(data);
						$("#contentData").html("");
						$("#contentData").append(data);
